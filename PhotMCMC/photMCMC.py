...
# Dump objCollection to pickle file for later use in analysis
    ...
    lenProd = int(nprod/thin)
    map_samples        = samples.reshape(nwalkers, lenProd, nparam)
    map_samples_lnprob = samples_lnprob.reshape(nwalkers, lenProd)
    max_ind        = np.argmax(map_samples_lnprob)
    max_ind        = np.unravel_index(max_ind, (nwalkers, lenProd))
    max_ind        = tuple(max_ind)
    theta        = map_samples[max_ind]
...
def dumpSed(objectName, paramFileName, sedFileName, teff, logg, Av, DM):
    ...
    print('# obj:', objectName, 'teff: ', teff, 'logg: ', logg, 'Av: ', Av, 'DM: ', DM, file=sedOut)
...
def postFromThetaFile(paramFileName, thetaFileName, outputFileName):
    ...
    theta = np.loadtxt(thetaFileName)

    logPost, blob = objCollection(theta)
    print(thetaFileName, logPost)
    
    print('#name teff logg Av DM, logPost=', logPost, file=fOut)


    for objName in objCollection.objNames:
        obj = objCollection.objPhot[objName]
        print(objName, obj.teff, obj.logg, obj.Av, obj.optDM)
